...
public class AccountServiceTest {
    ...
    void getTripsByAccountIdTest() {
        ...
        when(tripClient.getTripsByAccountId(accountId)).thenReturn(trips);

        List<TripDTO> result = accountService.getTripsByAccountId(accountId);


        assertNotNull(result);
        assertEquals(2, result.size());
        verify(tripClient, times(1)).getTripsByAccountId(accountId);
    }
